...
# Loads assemblys needed for GUI
    ...
    try {
        ...
        foreach ($user in $users) {
            try {
                ...
                Set-RemoteMailbox -Identity $user.samAccountName -UserPrincipalName $user.NewUPN -PrimarySmtpAddress $user.NewUPN -ErrorAction Stop -WarningAction SilentlyContinue
